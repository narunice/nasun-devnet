...
| Fullnode + Indexer   | Node 3 (m6i.2xlarge) — dedicated  |
...
│   ├── NASUN_DEVNET_OPERATIONS.md      # Operations guide
│   ├── NASUN_DEVNET_POST_RESET_CHECKLIST.md  # Post-reset checklist
│   └── NASUN_DEVNET_RESET_GUIDE.md     # Reset procedures
...
| nasun-node-3 | 54.180.61.196  | Fullnode (RPC) + sui-indexer + PostgreSQL + Explorer API + Nginx | m6i.2xlarge (32GB) | 300GB gp3 |
...
- **OOM 보호**: `OOMScoreAdjust=500`, `MemoryHigh=3G`, `MemoryMax=4G`
- **CPU 제한**: `CPUQuota=100%`
...
- [NASUN_DEVNET_POST_RESET_CHECKLIST.md](doc/NASUN_DEVNET_POST_RESET_CHECKLIST.md) - 리셋 후 체크리스트
- [NASUN_DEVNET_RESET_GUIDE.md](doc/NASUN_DEVNET_RESET_GUIDE.md) - 네트워크 리셋 가이드
...
상세한 리셋 절차: [NASUN_DEVNET_RESET_GUIDE.md](doc/NASUN_DEVNET_RESET_GUIDE.md) 참조
...
| Node 3 | - | - | **m6i.2xlarge (32GB)** — Fullnode + Indexer + Explorer |
| 아키텍처 | 2-node | 2-node | **3-node** |
| 월 비용 | ~$143.8 | ~$213.9 | **~$332** (RI 적용시 ~$241) |

### 배포된 컨트랙트 (V7)

> V7 genesis 이후 모든 컨트랙트 재배포 완료. Object ID는 "배포된 스마트 컨트랙트 (V6)" 섹션과 동일 (V7에서 재배포됨).
...
| Governance | ✅ 배포 완료 |
| Baram (AI Settlement) | ✅ 배포 완료 |
| Baram Executor | ✅ 배포 완료 |
...
| **Fullnode 자동 재시작** | 6시간 cron → 이후 4시간 → 현재 8시간 (0/8/16 UTC) | 메모리 leak 자동 관리, RSS 7-8GB → ~800MB |
...
| node-3 | 신규 m6i.2xlarge, Fullnode+Indexer+PostgreSQL+Explorer+Nginx |
