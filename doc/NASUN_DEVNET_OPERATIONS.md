...
2026-02-26에 발생한 연쇄 장애(Fullnode memory leak → indexer OOM → StartLimitBurst 소진 → .chk 306K 누적 → glob ARG_MAX 실패)의 재발을 방지하는 6-Layer 방어 체계.
...
0 * * * * /home/ubuntu/indexer-db-reinit.sh
```
...
**Layer 6: Auto DB Reinit (2026-03-04 추가)**

스크립트: `/home/ubuntu/indexer-db-reinit.sh` (매시간 cron)
- DB 크기 20GB+ 초과 시 자동 reinit 실행
- 6시간 cooldown (과도한 reinit 방지)
- 전체 절차 자동화: stop → drop/create → start → chain_identifier 삽입 → PM2 재시작
- Maintenance lock 공유 (watchdog와 충돌 방지)
- SNS 알림: reinit 실행/완료/실패 통보
- **배경**: SUI indexer pruning은 epoch 0(genesis) 시작이 필수이며, mid-epoch 시작 시 파티션 불일치로 crash. 따라서 pruning 대신 주기적 DB reinit으로 크기 관리
- **State 파일**: `/home/ubuntu/.indexer-db-reinit-state` (마지막 reinit epoch)
- **로그**: `/home/ubuntu/indexer-db-reinit.log`
...
| DB 크기 과다 (pruning 불가) | 자동 DB reinit + chain_identifier 삽입 (Layer 6) |
...
- **시작 스크립트**: `/home/ubuntu/nasun-node/start-indexer.sh` (systemd ExecStart)
  - DB에서 MAX(checkpoints) 읽어 resume, DB 비어있으면 oldest .chk 파일부터 시작
...
- **CPU 제한**: `CPUQuota=100%`, **메모리 제한**: `MemoryHigh=3G`, `MemoryMax=4G`
- **data-ingestion 자동 정리**: `--gc-checkpoint-files` (기본 true, 처리 완료 파일 자동 삭제)
- **Pruning**: 사용 불가 — SUI indexer pruning은 epoch 0(genesis)부터 시작해야 epoch-based 파티셔닝이 작동함. mid-epoch 시작 시 파티션 불일치로 crash
- **DB 성장률**: ~3.6GB/epoch (~43GB/day, 프루닝 없이). 300GB EBS에서 약 5-6일 후 DB reinit 필요
...
### 10.6 인덱서 DB Reinit (장애 복구 / 디스크 관리)

DB reinit은 (1) Devnet 전체 리셋, (2) indexer 장애 복구, (3) DB 크기 관리 시 수행합니다.
프루닝이 불가하므로 DB 크기가 threshold를 초과하면 reinit으로 정리합니다.
...
# 3. 인덱서 재시작 (start-indexer.sh가 자동으로 oldest .chk부터 시작)
sudo systemctl start sui-indexer

# 4. chain_identifier 삽입 (indexer가 schema 생성 후 실행)
# indexer 시작 후 30초 이상 대기 후 실행
DIGEST=$(curl -s http://localhost:9000 -X POST -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"sui_getCheckpoint","params":["0"]}' | \
  python3 -c "import sys,json; print(json.load(sys.stdin)['result']['digest'])")
HEX=$(python3 -c "
...
> **중요**: chain_identifier를 삽입하지 않으면 Explorer API의 chainId가 null이 됩니다.
> Indexer가 schema를 생성할 시간이 필요하므로 시작 후 30초 이상 대기하세요.
...
| indexer active이지만 체크포인트 진행 없음 | .chk 파일 gap (이전 인스턴스가 GC 후 crash) | DB reinit → start-indexer.sh가 oldest .chk부터 재시작 |
| Explorer API chainId: null | DB reinit 후 chain_identifier 테이블 비어있음 | 10.6절 chain_identifier 삽입 절차 수행 |
| pruning 활성 후 epoch boundary crash | mid-epoch 시작 시 epoch 파티션 불일치 | **pruning 사용 불가** — `--pruning-config-path` 제거, DB reinit |
| DB 크기 과다 (25GB+) | pruning 미동작 (mid-epoch 시작) | DB reinit으로 정리 (10.6절). 데이터는 .chk부터 재인덱싱 |
| start-indexer.sh Broken Pipe | `set -euo pipefail` + `sort \| head` SIGPIPE | `${VAR%%$'\n'*}` 패턴 사용 (파이프 회피) |
...
| 7.3.0 | 2026-03-04 | **Indexer DB 관리 강화**: start-indexer.sh 개선 (oldest .chk fallback, SIGPIPE 회피), pruning 비호환성 문서화 (mid-epoch 시작 불가), DB reinit 절차에 chain_identifier 복구 추가, Section 10.3/10.6/10.7 업데이트 | Claude Code |
| 7.4.0 | 2026-03-04 | **Auto DB Reinit (Layer 6)**: `indexer-db-reinit.sh` 구현 (매시간 cron, 20GB threshold, 6h cooldown, chain_identifier 자동 삽입, SNS 알림). Section 4.9에 Layer 6 추가, 장애 시나리오 커버리지 업데이트 | Claude Code |
